...
class Layanan extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $this->data['get_all']  = $this->Layanan_model->get_all();

    $this->load->view('back/layanan/layanan_list', $this->data);
  }

  public function create()
  {
    $this->data['title']          = 'Tambah '.$this->data['module'].' Baru';
    $this->data['action']         = site_url('admin/layanan/create_action');
    ...
    $this->load->view('back/layanan/layanan_add', $this->data);
  }
  ...
  public function create_action()
  {
    ...
    else
    {
      ...
      {
        $nmfile = strtolower(url_title($this->input->post('nama_layanan'))).date('YmdHis');
        ...
        if (!$this->upload->do_upload('foto'))
        {
          ...
          else
          {
            ...
            $config['source_image']   = './assets/images/layanan/'.$foto['file_name'].'';
            // rasio resolusi
  ...
  public function update($id)
  {
    ...
    $this->data['layanan'] = $this->Layanan_model->get_by_id($id);

    if ($row)
    {
      $this->data['title']          = 'Ubah Data '.$this->data['module'];
      $this->data['action']         = site_url('admin/layanan/update_action');
      $this->data['button_submit']  = 'Simpan';
      $this->data['button_reset']   = 'Reset';

      $this->data['id_layanan'] = array(
        'name'  => 'id_layanan',
        'id'    => 'id_layanan',
        'type'  => 'hidden',
      );
      ...
      $this->load->view('back/layanan/layanan_edit', $this->data);
    }
      else
      {
  ...
  public function update_action()
  {
    ...
    if ($this->form_validation->run() == FALSE)
    {
      $this->update($this->input->post('id_layanan'));
    }
      else
      {
        $nmfile = strtolower(url_title($this->input->post('nama_layanan'))).date('YmdHis');
        ...
        if ($_FILES['foto']['error'] <> 4)
        {
          ...
          if (!$this->upload->do_upload('foto'))
          {
            ...
            {
              $delete = $this->Layanan_model->del_by_id($this->input->post('id_layanan'));

              $dir        = "assets/images/layanan/".$delete->foto;
              ...
              $this->Layanan_model->update($this->input->post('id_layanan'), $data);
              $this->session->set_flashdata('message', '
              <div class="alert alert-block alert-success"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>
                <i class="ace-icon fa fa-bullhorn green"></i> Data berhasil disimpan
              </div>');
  ...
  {
    $delete = $this->Layanan_model->del_by_id($id);
  ...
  {
    $this->form_validation->set_rules('nama_layanan', 'Judul Layanan', 'trim|required');
