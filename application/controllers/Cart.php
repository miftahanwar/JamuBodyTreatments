...
class Cart extends CI_Controller
{
	...
	{
		$count = count($this->input->post('layanan'));
		for ($i = 0; $i < $count; $i++) {
			$data_detail[$i] = array(
	...
	public function download_invoice($id)
	{
		...
		if (!$row || $this->session->userdata('user_id') != $row->user_id) {
	...
	public function getJamMulai()
	{
		...
		if ($tanggal === FALSE || $layanan_id === FALSE) {
			echo json_encode(array());
			die();
		}

		$list_jam_mulai_terpakai = $this->Transaksi_detail_model->get_jam_mulai_terpakai($tanggal, $layanan_id);
